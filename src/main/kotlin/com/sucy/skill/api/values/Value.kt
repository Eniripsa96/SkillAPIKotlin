...
class Value {
    ...
    private val additiveStacks = mutableMapOf<String, ValueStacks>()
    ...
    fun addBonusStack(amount: Double, duration: Double, maxStacks: Int, type: TimerType, source: String) {
        val stacks = additiveStacks.computeIfAbsent(source) {
    ...
    fun setTo(value: Double, source: String) {
        baseSources.clear()
        multiplierSources.clear()
        additiveSources.clear()

        baseStacks.values.forEach(ValueStacks::cancel)
        additiveStacks.values.forEach(ValueStacks::cancel)
        multiplierStacks.values.forEach(ValueStacks::cancel)

        baseStacks.clear()
        additiveStacks.clear()
